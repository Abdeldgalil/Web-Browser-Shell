...
import { Plus, X, VenetianMask, RotateCcw } from 'lucide-react';
...
export default function TabSwitcher({ visible, onClose }: Props) {
  ...
  const { tabs, activeTabId, switchTab, closeTab, newTab, recentlyClosed, reopenClosedTab } = useBrowser();
  ...
  return (
    <div className="modal-backdrop" onClick={onClose}>
      ...
      >
        ...
        <div className="tabswitcher-scroll">
          ...
          {recentlyClosed.length > 0 && (
            <div className="recently-closed-section">
              <div className="recently-closed-title" style={{ color: colors.mutedForeground }}>
                Recently Closed
              </div>
              {recentlyClosed.map((c) => (
                <button
                  key={c.id}
                  className="recently-closed-row"
                  style={{ borderBottomColor: colors.border }}
                  onClick={() => {
                    reopenClosedTab(c.id);
                    onClose();
                  }}
                >
                  <RotateCcw size={16} strokeWidth={2} color={colors.mutedForeground} />
                  <div className="recently-closed-info">
                    <div className="recently-closed-name" style={{ color: colors.foreground }}>
                      {c.title}
                    </div>
                    <div className="recently-closed-url" style={{ color: colors.mutedForeground }}>
                      {getDisplayUrl(c.url)}
                    </div>
                  </div>
                </button>
